...
from ....attestation.noodle.block import EMPTY_SIG, GENESIS_HASH, GENESIS_SEQ, NoodleBlock, ValidationResult
...
class TestBlock(NoodleBlock):
    ...
    def __init__(self, transaction=None, previous=None, key=None, linked=None, block_type=b'test'):
        ...
        if previous:
            ...
            NoodleBlock.__init__(self, (block_type, json.dumps(transaction), previous.public_key,
...
class TestTrustChainBlock(unittest.TestCase):
    ...
    def test_create_genesis(self):
        ...
        db = MockDatabase()
        block = NoodleBlock.create(b'test', {'id': 42}, db, key.pub().key_to_bin(), link=None)
    ...
    def test_create_link_genesis(self):
        ...
        link = TestBlock()
        db.add_block(link)
        block = NoodleBlock.create(b'test', {b'id': 42}, db, key.pub().key_to_bin(), link=link)
        self.assertEqual(block.previous_hash, GENESIS_HASH)
        self.assertEqual(block.sequence_number, GENESIS_SEQ)
        self.assertEqual(block.public_key, key.pub().key_to_bin())
