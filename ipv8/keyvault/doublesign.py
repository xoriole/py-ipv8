...
def unpack32(s):
    if len(s) != 32:
        raise ValueError('Invalid scalar (len=%d)' % len(s))
...
class FiniteField(object):
    ...
    def div(self, lhs, rhs):
        # print "lhs:", lhs.value
        # print "rhs:", rhs.value
        # print (lhs.value * rhs.inverse()) % self.p
...
class ECPoint(object):
    """
    Represents a point in the EllipticCurve.
    """
...
class ECDSA(object):
    ...
    def custom_sign(self, hex_private_key, msg_str, secret_str):
        ...
        print "sign secret: %0x" % secret_hash_int

        # Generate the signature
        (r, s) = self.sign(msg_hash_int, private_key_int, secret_hash_int)
        # signature = pack32(r.value) + pack32(s.value)
        signature = "%064x%064x" % (r.value, s.value)
        return signature
    ...
    def custom_verify(self, msg_str, signature_str):
        ...
        r = int(signature_str[:length], 16)
        s = int(signature_str[length:], 16)

        # First hash the message
        msg_hash = hashes.Hash(hashes.SHA256(), backend=default_backend())
        msg_hash.update(msg_str)
        msg_hash_int = int(msg_hash.finalize().encode('hex'), 16)
    ...
    def pubkey_from_signatures(self, r1, s1, r2, s2, flag1, flag2):
        """
    ...
    def recover_from_double_signatures(self, msg1_str, msg2_str, signature1_str, signature2_str):
        ...
        r1 = int(signature1_str[:length], 16)
        s1 = int(signature1_str[length:], 16)
        r2 = int(signature2_str[:length], 16)
        s2 = int(signature2_str[length:], 16)
...
def test_dsa():
    ...
    verifytest(priv, dsa.privatekey_from_signsecret(r1, s1, msg1, signsecret1), "crack1")
    ...
    (crackedsecret, crackedprivkey) = dsa.recover_from_double_signature(r1, s1, s2, msg1, msg2)
    print "secret:%s, priv key: %s" % (crackedsecret, crackedprivkey)
    ...
    """
    for flag in range(2):
        pk1 = dsa.pubkey_from_signature(msg1, r1, s1, flag)
        check1 = dsa.verify(msg1, pk1, r1, s1)
        print "%d -> %s pk1=%s" % (flag, check1, pk1)

        pk2 = dsa.pubkey_from_signature(msg2, r2, s2, flag)
    ...
    for flag1 in range(2):
        for flag2 in range(2):
            pk = dsa.pubkey_from_signatures(r1, s1, r3, s3, flag1, flag2)
...
def test_dsa2():
    ...
    # Recover the private key and sign secret from these two signatures
    (crackedsecret, crackedprivkey) = dsa.recover_from_double_signature(r1, s1, s2, msg1, msg2)
    print "Private key: %s" % (crackedprivkey)
    print "Sign Secret: %s" % (crackedsecret)
...
def test_signature_verification():
    ...
    # Sign the first message and verify the signature
    signature = dsa.custom_sign(priv_hex, msg1, signsecret1)
    check1 = dsa.custom_verify(msg1, signature)
    verifytest(check1, True, "verify1")

    # Sign the second message and verify the signature
    signature2 = dsa.custom_sign(priv_hex, msg2, signsecret1)
    check2 = dsa.custom_verify(msg2, signature2)
    verifytest(check2, True, "verify1")

    # Recover the private key and sign secret from these two signatures
    (crackedsecret, crackedprivkey) = dsa.recover_from_double_signatures(msg1, msg2, signature, signature2)
    print "Private key: %s" % (crackedprivkey)
    print "Sign Secret: %s" % (crackedsecret)
