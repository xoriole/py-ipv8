"""
This twistd plugin starts a Noodle crawler.
"""
...
from ipv8.attestation.noodle.community import NoodleCrawlerCommunity
from ipv8.attestation.noodle.database import NoodleDB
...
from ipv8.attestation.noodle.settings import NoodleSettings
...
class NoodleCrawlerServiceMaker(object):
    tapname = "noodle_crawler"
    description = "Noodle crawler"
    options = Options

    def __init__(self):
        """
        Initialize the variables of the Noodle crawler and the logger.
        """
        self.ipv8 = None
        self.restapi = None
    ...
    def start_crawler(self, options):
        """
        Main method to startup the Noodle crawler.
        """
        ...
        # Open the database
        self.tc_persistence = NoodleDB(options["statedir"], 'noodle')
        ...
        if options["statedir"]:
            ...
            for community in crawler_config["overlays"]:
                if community["class"] == "NoodleCrawlerCommunity":
                    community["initialize"]["persistence"] = self.tc_persistence

        if 'testnet' in options and options['testnet']:
            for community in crawler_config["overlays"]:
                if community["class"] == "NoodleCommunity":
                    community["class"] = "NoodleTestnetCommunity"

        extra_communities = {
            "NoodleCrawlerCommunity": NoodleCrawlerCommunity,
        }
    ...
    def makeService(self, options):
        ...
        crawler_service.setName("NoodleCrawler")
...
service_maker = NoodleCrawlerServiceMaker()
